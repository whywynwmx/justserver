...
function shield:Init(words, tree)
	if self.inited then return end
	if words == nil then return end
	for _, word in pairs(words) do
		_word2Tree(tree or _tree, word.maskwords)
	end

	self.inited = true
end

function shield:AddWordlib(words, tree)
	for _, word in pairs(words) do
		_word2Tree(tree or _tree, word)
	end
end

function shield:string(s, tree)
	...
	while true do
    	word = utf8Sub(s, i)
    	idx = _detect(tree or _tree, word, i)
