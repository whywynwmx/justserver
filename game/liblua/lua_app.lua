...
-- 钩子函数，作用是统计每行的内存增长量
------------------------------
local function MemRecordAlloc(event, lineNo)
...
-- 挂上钩子
------------------------------
local function mem_record_start()
    memStat = {}
    currentMem = collectgarbage("count")
    debug.sethook(MemRecordAlloc, 'l')  
end

------------------------------
-- 摘掉钩子
------------------------------
local function mem_record_stop()
    debug.sethook()  
end

local function dumpFile(msg, ...)
	msg = string.format(msg, ...)
	io.write(msg)
end

local function mem_record_dump()    
    local sorted = { }
...
--[[
local oldYield = co_yield
local curCo = nil
local curCoCount = 0
local ttCoCount = 0

function lua_app.startCountCo()
	curCo = co_running()
	curCoCount = 0
	ttCoCount = 0
end

function lua_app.stopCountCo(msg)
	if curCoCount > 0 then
		print("stopCountCo", msg, curCoCount, ttCoCount)
	end
end

local co_yield = function(...)
	ttCoCount = ttCoCount + 1
	if curCo and curCo == co_running() then
		curCoCount = curCoCount + 1
	end
	return oldYield(...)
end
]]
...
function new_co(f)	
	...
	if co == nil then
		co = co_create(function(...)
			local inputs = {...}
			if (inputs and inputs[1] == "DELETE") then
				--一些定时器等需要跳过函数执行
			else
				f(...)
			end
	...
	--lua_app.coStacks[co] = debug.traceback()
	return co
end
...
function add_timer_ex(delay_ms,func,...)
	add_timer_cnt = add_timer_cnt + 1
	local args = {...}
	local session = clib.run_after(delay_ms)
...
function undo_timer(tt)
	if not tt or not tt.timer then
		return
	end
...
function do_timer(timems, func, obj, safeCall)
	if safeCall == nil then
		safeCall = true
	end

	local tt = {}
	local function _doTimer ()
        if tt.timer then
			lua_app.del_timer(tt.timer)
			tt.timer = nil
        end

		local continue = true
		if safeCall then
			local rt, msg = pcall(func, obj)
			if not rt then
				log_error(msg)
			else
				continue = msg
		...
		if continue ~= false then
			tt.timer = lua_app.add_timer(timems, _doTimer)
		end
	end
...
function second()
	return os.date("*t", lua_app.now()).sec
end

function daytime()
	local d = os.date("*t", lua_app.now())
...
function now0(t)
	...
	return lua_app.now() - hour()*3600 - minute()*60 - second()
end

-- 检查重复调用
local repeatCalls = {}
local function checkRepeatCall(key, func, obj)
	if repeatCalls[key] then
		lua_app.log_warning("checkRepeatCall Failed", key)
		return
	end

	repeatCalls[key] = true
	local rt = { pcall(func, obj) } 
	if not rt[1] then
		lua_app.log_warning("checkRepeatCall Error", rt[2])
		repeatCalls[key] = nil
		return
	end
...
local function safeWaitLockRun(lock, func, obj, waitprintcount)
	waitlockrun(lock, function()
		local rt, errmsg = pcall(func, obj)
		if not rt then
			lua_app.log_error(errmsg)
		end
	end, waitprintcount)	
end
...
lua_app.add_timer_ex = add_timer_ex
lua_app.del_timer = del_timer 
lua_app.do_timer = do_timer 
lua_app.undo_timer = undo_timer 
...
lua_app.now0 = now0
lua_app.week = week
lua_app.yearWeek = yearWeek
lua_app.hour = hour
lua_app.minute = minute
lua_app.second = second
lua_app.daytime = daytime
...
lua_app.traceback = traceback
lua_app.log_detail = log_detail
lua_app.mem_record_start = mem_record_start
lua_app.mem_record_stop = mem_record_stop
lua_app.mem_record_dump = mem_record_dump
