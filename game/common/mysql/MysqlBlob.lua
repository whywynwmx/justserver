...
function Blob:Init(cfg)
	...
	local function _Save()
		self.savetimer = skynet.timeout(savetime, _Save)
		self:Save()
	end
	self.savetimer = skynet.timeout(math.random(1, math.ceil(savetime/3))+savetime, _Save)
end
...
function Blob:CheckDB(dbconfig, config, ip, port, user, pass, dbname)
	...
	if not ret then
		...
		for i = ret.beforevs + 1, bfvs do
			skynet.log_info("Blob:CheckDB:: start update mysql before version", i)
			beforeversion[i](self, gconn)
			gconn:update(versionname, { server = server.wholename }, { beforevs = i })
			skynet.log_info("Blob:CheckDB:: end update mysql before version", i)
		end
	end
	skynet.log_info("Blob:CheckDB:: cur beforevs is", bfvs)
...
function Blob:CheckUpdate(updateconfig, dbconfig, ip, port, user, pass, dbname)
	...
	for i = ret.version + 1, nowvs do
		skynet.log_info("Blob:CheckUpdate:: start update mysql version", i)
			...
			for tbname, upinfo in pairs(updateconfig[i]) do
				...
				for j = 1, math.ceil(count/5000) do
					...
					for jj, oldvalue in ipairs(oldvalues) do
						...
						skynet.log_info("  || update", tbname, (j - 1)*5000 + jj, sql)
		...
		skynet.log_info("Blob:CheckUpdate:: end update mysql version", i)
	end
	skynet.log_info("Blob:CheckUpdate:: cur version is", nowvs)
	gconn:wait_close()
...
function Blob:DelCache(tbname, cache)
	if not self.cachelist[tbname] or not self.cachelist[tbname][cache] or self.cachelist[tbname][cache] == _DataType.Del then
		skynet.log_error("Blob:SetCacheType:: no cache", tbname, cache)
		table.ptable(cache)
		return
	end
...
function Blob:UpdateCache(tbname, cache)
	-- if not self.cachelist[tbname] or not self.cachelist[tbname][cache] or self.cachelist[tbname][cache] == _DataType.Del then
	-- 	skynet.log_error("Blob:SetCacheType:: no cache", tbname, cache, datatype)
	-- 	table.ptable(cache)
	-- 	return
	-- end
	...
	local _mt = {
		...
		__newindex = function(tb, key, value)
			if cache[key] == nil or value == nil then
				skynet.log_error("no cache key or value", key, value, tbname)
				return
			end
			cache[key] = value
			self:UpdateCache(tbname, cache)
...
function Blob:LoadUniqueDmg(tbname, tbcolumn)
	...
	local _mt = {
		...
		__newindex = function(tb, key, value)
			...
			-- 	skynet.log_error("no cache key or value", key, value, tbname, tbcolumn)
			-- 	return
			-- end
			column[key] = value
			self:UpdateCache(tbname, cache)
...
function Blob:Save()
	if self.isinsave then
		skynet.log_error("Blob:Save isinsave", self.isinsave)
		return
	end

    local startTime = skynet.time_ms()

	self.isinsave = skynet.timeI()
    ...
    local overTime = skynet.time_ms()
	if overTime - startTime > 1000 then
		skynet.log_error("Cost Time Too Long in Blob:Save", overTime - startTime)
	end
	self.isinsave = nil
end
...
function Blob:Release()
	if self.savetimer and self.savetimer ~= 0 then
		skynet.remove_timeout(self.savetimer)
		self.savetimer = nil
	end
	local waitcount = 0
	while self.isinsave do
		...
		if waitcount % 50 then
			skynet.log_info("Blob:Release isinsave, waitcount", self.isinsave, waitcount)
		end
	end
	-- -- 关服强制全部存一次
