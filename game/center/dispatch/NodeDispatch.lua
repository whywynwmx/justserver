...
function NodeDispatch:SetNode(node, name, index)
	...
	skynet.log_info("NodeDispatch:SetNode::", node, name, index)
end
	...
	for name, nodelist in pairs(self.nodelist) do
		...
		if indexs then
			...
			for index, _ in pairs(indexs) do
				...
				skynet.log_info("NodeDispatch:RemoveNode::", node, name, index)
			end
		end
	end
...
function NodeDispatch:MoveNode(node, tonode)
	...
	for name, nodelist in pairs(self.nodelist) do
		...
		if indexs then
			...
			for index, _ in pairs(indexs) do
				servicelist[index] = nil
				skynet.log_info("NodeDispatch:MoveNode::", node, name, index, tonode)
...
function NodeDispatch:RemoveOneNodeIndex(node, name, index)
	...
	skynet.log_info("NodeDispatch:RemoveOneNodeIndex::", node, name, index)
end
...
function NodeDispatch:ClearDtb()
	...
	skynet.log_info("NodeDispatch:ClearDtb")
end
...
function NodeDispatch:CheckNodeEnough(_, waitcheck)
	...
	if self.checkNodeTimer then
		skynet.remove_timeout(self.checkNodeTimer)
	...
	if waitcheck then
		for name, _ in pairs(server.dispatchCenter.dtblist) do
			if not self.nodes[server.serverConfig.svrNameToNodeName[name]] then
				self.checkNodeTimer = skynet.add_update_timer(5 * 100, self, "CheckNodeEnough", waitcheck)
	...
	if next(nodes) then
		...
		skynet.waitmultrun(function(_, info)
...
function NodeDispatch:SetNodeNumServer()
	if self.checkNodeNumTimer then
		skynet.remove_timeout(self.checkNodeNumTimer)
	...
	for nname, num in pairs(server.GetNodeNum()) do
		...
		if count < num then
			self.checkNodeNumTimer = skynet.add_update_timer(5 * 100, self, "SetNodeNumServer")
