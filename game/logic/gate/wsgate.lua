...
local protoSender = nil
local protoHoster = nil 
...
function handle.connect(fd)
    skynet.error("新的连接建立: fd=%d", fd)
end

function handle.handshake(fd, header, url)
    local addr = websocket.addrinfo(fd)
    skynet.log_info("websocket握手成功", addr)
    connections[fd] = 0
...
function handle.message(fd, msg, msg_type)
    ...
    if player then
        ...
        local msgtype, name, args, response = protoHoster:dispatch(msg, size)
        ...
        else --if msgtype == "RESPONSE" then
            skynet.error("error, server not support response", msgtype)
            close_internal(fd)
            return
        end
...
function CMD.start(conf)
    server.index = conf.zone_id
    server.wholename = server.GetWholeName(server.name, server.index, server.platformid)
    skynet.log_info("wsgate start:", lua_util.inspectex(conf))
    init(conf)
    skynet.log_info("wsgate try listen:", conf.port)
    local listenfd = socket.listen("0.0.0.0", conf.port)
    skynet.log_info("wsgate listen:", conf.port, listenfd)
