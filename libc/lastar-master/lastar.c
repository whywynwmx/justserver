...
// static int get_earlyExit(lua_State *L, int ref,
// 			 int visitedCount, int h,
// 			 int sx, int sy,
// 			 int ex, int ey,
// 			 float *dis)
// {
...
static int path_earlyExit(size_t visitedCount, void *visitingNode, void *goalNode, void *context)
{
	map_t *map = (map_t *)context;
	lua_State *L = (lua_State *)map->udata;
...
static int lua__new_map(lua_State *L)
{
	...
	if (!lua_isfunction(L, 3)) {
		return luaL_error(L, "create map failed, no neighbor function");
	}
...
// static const struct luaL_Reg tg_funcs[] = {
//     {"path", lua__path},
// 	{"new", lua__new_map},
// 	{"__gc", lua__gc},
//     { NULL, NULL }
...
// int luarr_lastar(lua_State* L) 
// {
//     luaL_newlib(L, tg_funcs);
//     return 1;
// }

// int luaopen_lastar(lua_State* L)
// {
//     luaL_requiref(L, "lstar", luarr_lastar, 1);
